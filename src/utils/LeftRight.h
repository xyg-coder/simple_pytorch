...
struct IncrementRAII final {
 ...
 private:
  ...
  IncrementRAII(const IncrementRAII&) = delete;
  IncrementRAII& operator=(const IncrementRAII&) = delete;
};
...
// C++ expert.
//
// my own understanding: this is suitable to be used when the write and 
// read are unlikely to happen in the same time. So this won't affect the reading
//
template <class T>
class LeftRight final {
 public:
  ...
  auto read(F&& readFunc) const {
    IncrementRAII _increment_counter(
